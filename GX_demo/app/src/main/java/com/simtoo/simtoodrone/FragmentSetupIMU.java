...
public class FragmentSetupIMU extends ApiListenerFragment  {
    ...
    static {
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_IMU_ERROR);

        intentFilter.addAction(Constans.AttributeEvent.STATE_CONNECTED);
        intentFilter.addAction(Constans.AttributeEvent.STATE_DISCONNECTED);
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_MAG_STARTED);
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_MAG_ESTIMATION);
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_MAG_COMPLETED);
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_IMU);
        intentFilter.addAction(Constans.AttributeEvent.HEARTBEAT_TIMEOUT);
        intentFilter.addAction(Constans.AttributeEvent.CALIBRATION_IMU_TIMEOUT);
    }
    ...
    private final BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(Constans.AttributeEvent.CALIBRATION_IMU.equals(action)){
                String message = intent.getStringExtra(Constans.AttributeEventExtra.EXTRA_CALIBRATION_IMU_MESSAGE);

                if(message != null)
                    processMAVMessage(message, true);
            }
            else if(Constans.AttributeEvent.STATE_CONNECTED.equals(action)){
            ...
            else if(Constans.AttributeEvent.STATE_DISCONNECTED.equals(action)){
                //Reset the screen, and disable the calibration button
                btnStep.setEnabled(false);
                resetCalibration();
            }
            else if (Constans.AttributeEvent.CALIBRATION_IMU_TIMEOUT.equals(action)) {
                //                LogUtil.d("333", "调用drone.isConnected()_19");
                if (getDrone().getState().isConnected()) {
                    String message = intent.getStringExtra(Constans.AttributeEventExtra.EXTRA_CALIBRATION_IMU_MESSAGE);
                }
            }
            else if(Constans.AttributeEvent.CALIBRATION_IMU_ERROR.equals(action)){
                String message = intent.getStringExtra(Constans.AttributeEventExtra.EXTRA_CALIBRATION_IMU_MESSAGE);
                if(message != null){
    ...
    private void sendAck(int step) {
        STDrone dpApi = getDrone();
        if (getDrone().getState().isConnected()) {
            dpApi.getCalibrationSetup()
                    .sendAckk(step);
    ...
    private void startCalibration() {
        STDrone dpApi = getDrone();
        MavLinkCalibration.sendStartAccelerometerMessages(dpApi);
    }
